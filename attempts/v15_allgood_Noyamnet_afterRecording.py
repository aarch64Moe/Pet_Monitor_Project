...
def run_real_time_classification():
    ...
    classification_timer = time.time()  # Initialize the timer
    ...
    try:
        with sd.InputStream(device=device_index, callback=audio_callback, channels=1, samplerate=sample_rate,
            ...
            while not recording:  # Run only when recording is inactive
                # Guard real-time classification with a 1-second timer
                if time.time() - classification_timer >= 1.0:  # Check if 1 second has passed
                    #print("Processing 1-second audio segment...")
